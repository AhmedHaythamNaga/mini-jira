...
let ProjectsService = class ProjectsService {
    ...
    async findOne(projectId) {
        const result = await this.dynamo.send(new lib_dynamodb_2.GetCommand({ TableName: this.tableName, Key: { projectID: projectId } }));
        if (!result.Item)
    ...
    async remove(projectId) {
        await this.findOne(projectId);
        await this.dynamo.send(new lib_dynamodb_2.DeleteCommand({ TableName: this.tableName, Key: { projectID: projectId } }));
        return { deleted: true };
    }
};
