...
let TasksService = class TasksService {
    ...
    async findOne(taskId) {
        const result = await this.dynamo.send(new lib_dynamodb_2.GetCommand({ TableName: this.tableName, Key: { taskId } }));
        if (!result.Item)
    ...
    async remove(taskId) {
        ...
        await this.dynamo.send(new lib_dynamodb_2.DeleteCommand({ TableName: this.tableName, Key: { taskId } }));
        return { deleted: true };
    }
